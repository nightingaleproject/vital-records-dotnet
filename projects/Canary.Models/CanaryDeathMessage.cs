...
namespace Canary.Models
{
    public class CanaryDeathMessage : Message, ICanaryDeathMessage
    {
        ...
        public CanaryDeathMessage() { }

        public CanaryDeathMessage(string message)
        {
        ...
        public override Dictionary<string, Message> GetResponsesFor(String type)
        {
            ...
            AcknowledgementMessage ack = new AcknowledgementMessage((BaseMessage) message);
            Message ackMsg = new CanaryDeathMessage(ack);
            result.Add("ACK", ackMsg);

            // Create the extraction error
            ExtractionErrorMessage err = new ExtractionErrorMessage((BaseMessage) message);
            // Add the issues found during processing
            var issues = new List<VRDR.Issue>();
            var issue = new VRDR.Issue(OperationOutcome.IssueSeverity.Fatal, OperationOutcome.IssueType.Invalid, "This is a fake message");
            issues.Add(issue);
            err.Issues = issues;
            Message errMsg = new CanaryDeathMessage(err);
            ...
            switch (type)
            {
                ...
                case "http://nchs.cdc.gov/vrdr_submission_update":
                    ...
                    Message mreMsg = new CanaryDeathMessage(mre);
                    ...
                    Message trxMsg = new CanaryDeathMessage(trx);
