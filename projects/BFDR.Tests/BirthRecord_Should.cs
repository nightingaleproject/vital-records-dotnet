...
namespace BFDR.Tests
{
  public class BirthRecord_Should
  {
    ...
    public void TestFullBirthRecordImport()
    {
      ...
      Assert.Equal(null, birthRecord.MotherSuffix);
      Assert.Equal(null, birthRecord.FatherSuffix);
      Assert.Equal(null, birthRecord.MotherMaidenSuffix);
      ...
      Assert.Equal(null, birthRecord.PluralityEditFlagHelper);
      Assert.Equal(1, birthRecord.Plurality);
      Assert.Equal(false, birthRecord.NoCongenitalAnomaliesOfTheNewborn);
      Assert.Equal(false, birthRecord.Anencephaly);
      Assert.Equal(false, birthRecord.CleftLipWithOrWithoutCleftPalate);
      Assert.Equal(false, birthRecord.CleftPalateAlone);
      Assert.Equal(false, birthRecord.CongenitalDiaphragmaticHernia);
      Assert.Equal(true, birthRecord.CyanoticCongenitalHeartDisease);
      Assert.Equal(true, birthRecord.DownSyndrome);
      Assert.Equal(false, birthRecord.Gastroschisis);
      Assert.Equal(false, birthRecord.Hypospadias);
      Assert.Equal(false, birthRecord.LimbReductionDefect);
      Assert.Equal(false, birthRecord.Meningomyelocele);
      Assert.Equal(false, birthRecord.Omphalocele);
      Assert.Equal(false, birthRecord.SuspectedChromosomalDisorder);
      Assert.Equal(false, birthRecord.NoCharacteristicsOfLaborAndDelivery);
      Assert.Equal(true, birthRecord.EpiduralOrSpinalAnesthesia);
      Assert.Equal(true, birthRecord.AntibioticsAdministeredDuringLabor);
      Assert.Equal(false, birthRecord.AugmentationOfLabor);
      Assert.Equal(true, birthRecord.Chorioamnionitis);
      Assert.Equal(true, birthRecord.InductionOfLabor);
      Assert.Equal(false, birthRecord.AdministrationOfSteroidsForFetalLungMaturation);
      Assert.Equal(false, birthRecord.NoSpecifiedAbnormalConditionsOfNewborn);
      Assert.Equal(false, birthRecord.NICUAdmission);
      Assert.Equal(true, birthRecord.AntibioticForSuspectedNeonatalSepsis);
      Assert.Equal(false, birthRecord.AssistedVentilationFollowingDelivery);
      Assert.Equal(false, birthRecord.AssistedVentilationMoreThanSixHours);
      Assert.Equal(false, birthRecord.Seizure);
      Assert.Equal(false, birthRecord.SurfactantReplacementTherapy);
      Assert.Equal(false, birthRecord.NoInfectionsPresentDuringPregnancy);
      Assert.Equal(true, birthRecord.Chlamydia);
      Assert.Equal(false, birthRecord.Gonorrhea);
      Assert.Equal(false, birthRecord.HepatitisB);
      Assert.Equal(false, birthRecord.HepatitisC);
      Assert.Equal(false, birthRecord.Syphilis);
      Assert.Equal(false, birthRecord.GenitalHerpesSimplex);
      Assert.Equal(false, birthRecord.NoMaternalMorbidities);
      Assert.Equal(false, birthRecord.ICUAdmission);
      Assert.Equal(false, birthRecord.MaternalTransfusion);
      Assert.Equal(false, birthRecord.PerinealLaceration);
      Assert.Equal(true, birthRecord.RupturedUterus);
      Assert.Equal(false, birthRecord.UnplannedHysterectomy);
      Assert.Equal(false, birthRecord.NoPregnancyRiskFactors);
      Assert.Equal(false, birthRecord.EclampsiaHypertension);
      Assert.Equal(false, birthRecord.GestationalDiabetes);
      Assert.Equal(true, birthRecord.GestationalHypertension);
      Assert.Equal(true, birthRecord.PrepregnancyDiabetes);
      Assert.Equal(false, birthRecord.PrepregnancyHypertension);
      Assert.Equal(false, birthRecord.PreviousCesarean);
      Assert.Equal(false, birthRecord.PreviousPretermBirth);
      Assert.Equal(true, birthRecord.ArtificialInsemination);
      Assert.Equal(false, birthRecord.AssistedFertilization);
      Assert.Equal(true, birthRecord.InfertilityTreatment);
      Assert.Equal(false, birthRecord.UnknownFinalRouteAndMethodOfDelivery);
      tempDict = new();
      ...
      Assert.Equal(true, birthRecord.LaborTrialAttempted);
      ...
      Assert.Equal(false, birthRecord.NoObstetricProcedures);
      Assert.Equal(false, birthRecord.SuccessfulExternalCephalicVersion);
      Assert.Equal(true, birthRecord.UnsuccessfulExternalCephalicVersion);
      ...
      Assert.Equal(null, birthRecord.MotherDateOfBirthEditFlagHelper);
      ...
      Assert.Equal(null, birthRecord.FatherDateOfBirthEditFlagHelper);
      tempDict = new();
      tempDict.Add("code", "Y");
      tempDict.Add("system", "http://terminology.hl7.org/CodeSystem/v2-0136");
      tempDict.Add("display", "Yes");
      ...
      Assert.Equal(true, birthRecord.InfantBreastfedAtDischarge);
      ...
      Assert.Equal(null, birthRecord.AttendantOtherHelper);
      ...
      Assert.Equal("Y", birthRecord.MotherTransferredHelper);
      Assert.Equal(true, birthRecord.InfantLiving);
      tempDict = new();
      ...
      Assert.Equal(true, birthRecord.SSNRequested);
      Assert.Equal(7, birthRecord.ApgarScoreFiveMinutes);
      Assert.Equal(null, birthRecord.ApgarScoreTenMinutes);
      ...
      Assert.Equal(null, birthRecord.EmergingIssue1_1);
      Assert.Equal(null, birthRecord.EmergingIssue1_2);
      Assert.Equal(null, birthRecord.EmergingIssue1_3);
      Assert.Equal(null, birthRecord.EmergingIssue1_4);
      Assert.Equal(null, birthRecord.EmergingIssue1_5);
      Assert.Equal(null, birthRecord.EmergingIssue1_6);
      Assert.Equal(null, birthRecord.EmergingIssue8_1);
      Assert.Equal(null, birthRecord.EmergingIssue8_2);
      Assert.Equal(null, birthRecord.EmergingIssue8_3);
      Assert.Equal(null, birthRecord.EmergingIssue20);
    }
