...
import { EDRSRoundtripConsuming } from '../EDRSRoundtripConsuming';
...
it('renders', () => {
  const tree = renderer.create(
    <MemoryRouter>
      <EDRSRoundtripConsuming params={{ "recordType": "bfdr-birth" }} />
    </MemoryRouter>
  ).toJSON();
...
it('should show the success message when issues is defined but empty', async () => {
  ...
  textArea.focus();
  await userEvent.paste('VALID IJE');
...
it('should show the problems when there are issues', async () => {
  ...
  const textArea = await component.findByRole('textbox');
  textArea.focus();
  await userEvent.paste('THIS IS NOT IJE');
  fireEvent.click(submitButton);
