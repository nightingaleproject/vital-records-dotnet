...
namespace canary.Controllers
{
    public static class ControllerMappers
    {
        ...
        public static readonly Dictionary<string, Func<string, bool, (Record, List<Dictionary<string, string>>)>> checkGetRecord = new() {
            {VRDR, static (string input, bool getFsh) => {
                    Record record = CanaryDeathRecord.CheckGet(input, false, out List<Dictionary<string, string>> issues, getFsh);
                    return (record, issues);
                }
            },
            {BFDR_BIRTH, static (string input, bool getFsh) => {
                    Record record = CanaryBirthRecord.CheckGet(input, false, out List<Dictionary<string, string>> issues);
                    return (record, issues);
                }
            },
            {BFDR_FETALDEATH, static (string input, bool getFsh) => {
